...
int main(int argc, const char* argv[])
{
    ...
    if (argc != 4)
    {
        fprintf(stderr, "Usage: %s <buffer size> <message count> <kernel buffer size>\n", argv[0]);
	return 1;
    }
    ...
    if (res <= 0)
    {
        fprintf(stderr, "Invalid pipe internal buffer size\n");
	return 1;
    }
    pipe_sz = (size_t)res;
    fprintf(stderr, " [.] Pipe internal buffer size: %zu\n", msg_count);
    ...
    if (pipe(pipes) != 0)
    {
        fprintf(stderr, " [!] Unable to create pipes: code %d\n", errno);
	return errno;
    }

    res = fcntl(pipes[0], F_GETPIPE_SZ);
    if (res <= 0) {
        fprintf(stderr, " [!] Unable to query F_GETPIPE_SZ: code %d\n", errno);
	return errno;
    }
    fprintf(stderr, " [.] Default kernel buffer size: %lld\n", res);
    res = fcntl(pipes[0], F_SETPIPE_SZ, pipe_sz);
    if (res <= 0) {
        fprintf(stderr, " [!] Unable to set F_GETPIPE_SZ: code %d\n", errno);
	return errno;
    }
    fprintf(stderr, " [.] Kernel buffer size: %zu\n", pipe_sz);
	...
	for (size_t i = 0; ; i++)
	{
	    ...
	    while (total_received != buf_size) {
	    	ssize_t bytes = read(pipes[0], buf, buf_size - total_received);
	    	if (bytes == 0) {
                    close(pipes[0]);
        ...
        for (size_t i = 0; i < msg_count; i++)
	{
	    ...
	    ssize_t bytes = write(pipes[1], buf, buf_size);
	    if (bytes != buf_size)
	    {
